...
export default function PlayerBar({ 
  ...
  if (!currentSong) {
    return (
      <div className="fixed bottom-0 left-0 right-0 bg-tokyo-bg-hl border-t border-tokyo-border p-4 z-50">
        <div className="flex items-center justify-center text-tokyo-comment">
  ...
  return (
    <>
      <audio ref={audioRef} />
      <div className="fixed bottom-0 left-0 right-0 bg-tokyo-bg-hl border-t border-tokyo-border p-2 sm:p-4 z-50">
        <div className="flex flex-col gap-2 sm:gap-3 z-50">
          ...
          <div className="flex items-center gap-3 w-full">
            ...
            <div className="min-w-0 flex-1">
              <p className="text-tokyo-fg text-sm font-semibold truncate">
                {currentSong.title}
              </p>
              <p className="text-tokyo-comment text-xs truncate">
            ...
            <div className="hidden sm:flex items-center gap-2">
              <button onClick={toggleMute} className="text-tokyo-comment hover:text-tokyo-fg">
                {isMuted || volume === 0 ? (
                  <VolumeX size={20} />
                ) : (
                  <Volume2 size={20} />
          ...
          <div className="flex flex-col items-center gap-2 w-full">
            <div className="flex items-center gap-4 justify-center">
              ...
              <button
                onClick={togglePlay}
                className="bg-tokyo-fg rounded-full p-2 sm:p-2.5 hover:scale-110 transition-transform"
              >
                {isPlaying ? (
                  <Pause size={20} fill="#1a1b26" className="text-tokyo-bg sm:w-6 sm:h-6" />
                ) : (
                  <Play size={20} fill="#1a1b26" className="text-tokyo-bg ml-0.5 sm:w-6 sm:h-6" />
                )}
              </button>
            ...
            <div className="w-full flex items-center gap-2">
              <span className="text-xs text-tokyo-comment w-8 sm:w-10 text-right">
                {formatTime(currentTime)}
              </span>
              <div className="flex-1 relative h-5 flex items-center group">
                <div className="absolute w-full h-1 bg-tokyo-fg-gutter rounded-full" />
                <div
                  className="absolute h-1 bg-tokyo-blue rounded-full pointer-events-none"
              ...
              <span className="text-xs text-tokyo-comment w-8 sm:w-10">
                {formatTime(duration)}
              </span>
            </div>
          </div>
